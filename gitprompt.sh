...
function updatePrompt() {
  ...
  if [[ -n "${GitStatus}" ]]; then
    ...
    PS1="${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]: ${PROMPT_START}$($prompt_callback)${STATUS}${PROMPT_END}"
    if [[ -n "${VIRTUAL_ENV}" ]]; then
      PS1="${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]: (${Blue}$(basename ${VIRTUAL_ENV})${ResetColor}) ${PS1}"
    fi

  else
    PS1="${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]: ${EMPTY_PROMPT}"
  fi
}
